  ...
  async function fetchData() {
    try {
      let res = await axios.get(
        `https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&id=${videoId}&key=${API_KEY2}`
      );
      let sub = await axios.get(
        ` https://www.googleapis.com/youtube/v3/channels?part=statistics&id=UCWX0cUR2rZcqKei1Vstww-A&key=${API_KEY2}`
      );
      ...
      // Earnings = min(subscriber count, views) + 10 * comments + 5 * likes
      {
        ...
        let commentCount = parseInt(res.data.items[0].statistics?.commentCount);
  ...
  return (
    <div>
          ...
          <div className="col-md-6 right-container">
            <div className="card">
              <div className="card-body right-body">
                ...
                <button className="btn btn-light mt-2 rounded-pill ">
                  check how?
                </button>
              </div>
            </div>
      ...
      <div
        ...
        style={{ backgroundColor: "black", padding: 0, marginTop: 20 }}
      >
        <h2 style={{ color: "#FFFFFFB2", fontSize: "28px" }}>
